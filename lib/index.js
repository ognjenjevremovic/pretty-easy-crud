...
function DatabaseCRUD(param_one, param_two) {
    ...
    if (param_one && !param_two) {
        ...
        if (!_isCollection._error) {
            collection =    _isCollection;
        } else if (typeof param_one === 'string' || typeof param_one === 'number' ||
    ...
    if (collection && !collection._error && collection.collection && _includeLogs(settings.includeLogs, 'info')) {
        moreInfo =  {};
        moreInfo.collectionName =   collection.name;
        moreInfo.dbName         =   collection.dbName;
